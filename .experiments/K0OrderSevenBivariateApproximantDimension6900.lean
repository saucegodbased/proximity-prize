...
four-gradient symbol faithful on the order-seven covariant space.  The
remaining integration obligation is pointwise boundary surjectivity after
choosing four vectors in the 34-dimensional kernel; the dimension receipt
alone must not be reported as that pointwise theorem.
-/
