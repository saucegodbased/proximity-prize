...
/-! Cheap original-factor count for `W=133225`, valid for every node card at most `262142`. -/
namespace ProximityPrize.SubmissionLower.WeightedCutoff2CheapRemainderCountW1332256900
...
open WeightedFactorFlagBudgetW1332246900 WeightedBatchFactorChoice6900
...
theorem cheapReducedCut_in_flag (F : MvPolynomial (Fin 4) K)
    ...
    (hJ : MvPolynomial.weightedTotalDegree ![0,1,1,1] F≤211) (x u : K) :
    PolynomialInFlag cheapAgreement (cheapReducedCut F x u) := by
  have hTdegree : F.degreeOf 3≤27 := by
    rw [← WeightedFactorFlagBudgetW1332246900.T_weight_eq_degreeOf]
    exact hT
...
theorem cheap_regular_scaled [CharP K 2130706433]
  ...
  have hsource : PolynomialInFlag cheapFlag (rtySource F) :=
    WeightedFactorFlagBudgetW1332246900.rtySource_in_flag_of_weights F cheapFlag hT hRT hJ
  have hav : a-v≤nodes.card-v := Nat.sub_le_sub_right hAn v
  have hsmallCurve : small*(a-v)≤(nodes.card-v)*cheapAgreement.all := by
    calc
      small*(a-v) ≤ cheapAgreement.all*(a-v) :=
        Nat.mul_le_mul_right _ (by norm_num [small,cheapAgreement,honestReducedAgreementFlag,w])
      _ = (a-v)*cheapAgreement.all := Nat.mul_comm _ _
      _ ≤ (nodes.card-v)*cheapAgreement.all := Nat.mul_le_mul_right _ hav
  have hsq : (a-v)^2≤(nodes.card-v)^2 := Nat.pow_le_pow_left hav 2
  have hsmallSurface : small*(a-v)^2≤(nodes.card-v)^2*cheapAgreement.all^2 := by
    calc
      small*(a-v)^2 ≤ cheapAgreement.all^2*(a-v)^2 :=
        Nat.mul_le_mul_right _ (by norm_num [small,cheapAgreement,honestReducedAgreementFlag,w])
      _ = (a-v)^2*cheapAgreement.all^2 := Nat.mul_comm _ _
      _ ≤ (nodes.card-v)^2*cheapAgreement.all^2 := Nat.mul_le_mul_right _ hsq
  have hactual := original_regular_of_congruent_active_flags F hF Gamma nodes x u hinj
    prime w a v small (by norm_num [w]) (by norm_num [w,prime])
    (by norm_num [prime]) (by norm_num [w,a]) (by norm_num [v,a]) hAn
    ...
    hsmallCurve hsmallSurface source_and_projection_gates.1
    (by norm_num [cheapFlag,cheapAgreement,honestReducedAgreementFlag,w,prime])
  exact hactual.trans (Nat.mul_le_mul_right _
    (Nat.pow_le_pow_left (Nat.sub_le_sub_right hn v) 2))
...
theorem cheap_remainder_scaled [CharP K 2130706433]
    ...
    (hJ : jetDegree (∏ F∈s,F)≤211) (hrho : derivativeDegree (∏ F∈s,F)≤55)
    (Gamma : Finset (Polynomial K)) (nodes : Finset I) (x u : I → K)
    (hinj : Set.InjOn x nodes) (hn : nodes.card≤n) (hAn : a≤nodes.card)
    (hdegree : ∀ P∈Gamma,P.natDegree≤w)
    (hagreement : ∀ P∈Gamma,a≤(nodes.filter fun i => P.eval (x i)=u i).card)
  ...
  have hweights := WeightedFactorFlagBudgetW1332246900.cheap_product_weight_bounds product hJ hrho
  have hfactor (F : ↑s) :
    ...
    exact cheap_regular_scaled F.1 (hirr F.1 F.2).ne_zero hT hRT hTotal
      (regularFamily F.1 Gamma) nodes x u hinj hn hAn
  ...
  have hsum := WeightedFactorFlagBudgetW1332246900.exactFlag_cumulative_of_weight_bounds
    s product hproduct dvd_rfl cheapFlag hweights.1 hweights.2.1 hweights.2.2
...
theorem cheap_remainder_card_le [CharP K 2130706433]
  ...
  exact count_le_of_scaled _ _ _ _ (by norm_num [a,v])
    (cheap_remainder_scaled Q s hs hJ hrho Gamma nodes x u hinj hn hAn
      hdegree hagreement hcommon) active_scaled_caps.1

end
end ProximityPrize.SubmissionLower.WeightedCutoff2CheapRemainderCountW1332256900

#print axioms ProximityPrize.SubmissionLower.WeightedCutoff2CheapRemainderCountW1332256900.cheap_remainder_card_le
