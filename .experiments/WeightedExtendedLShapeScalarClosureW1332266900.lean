import WeightedExtendedLShapeClosedW1332266900
...
/-!
# Scalar closure of the W=133226 hard identity branch
...
`(n,w,a;M,D,T)=(262144,133226,180413;744,244,122)` profile, derives the
...
-/
namespace ProximityPrize.SubmissionLower.WeightedExtendedLShapeScalarClosureW1332266900
...
def positiveAuxY : Nat := (2*122-1)*744
def positiveAuxR : Nat := (2*122-1)*244
def positiveRegularCap : Nat := 15606792282767367
def positiveSingularCap : Nat := 21438857736
def tFreeRegularCap : Nat := 264032710017
def tFreeSingularCap : Nat := 362328

def cleanupCap : Nat :=
  WeightedIdentityExtendedLShapeArithmeticW1332266900.primaryCleanupCap

theorem cleanup_cap_exact : cleanupCap=15607077754697448 := by
  exact WeightedIdentityExtendedLShapeArithmeticW1332266900.cleanup_exact

theorem cleanup_component_arithmetic :
    positiveAuxY=180792 ∧ positiveAuxR=59292 ∧
    firstOrderRegularNumerator positiveAuxY positiveAuxR≤
      positiveRegularCap*(a-w) ∧
    firstOrderRegularNumerator 744 244≤tFreeRegularCap*(a-w) ∧
...
/-- Exact W133226 semantic cleanup.  The T-free family already includes its
...
theorem actual_nonactive_bound {I : Type} [Fintype I]
    ...
    (hY : Q.degreeOf 1≤744) (hR : Q.degreeOf 2≤244)
    (hT : Q.degreeOf 3≤122)
    (Gamma : Finset (Polynomial K)) (nodes : Finset I) (x u : I→K)
    (hinj : Set.InjOn x nodes) (hn : nodes.card=n)
    (hdegree : ∀ P∈Gamma,P.natDegree≤w)
  ...
  have hJY : (tFreeProduct Q).degreeOf (1 : Fin 4)≤744 :=
...
theorem primary_coordinate_caps (Q : MvPolynomial (Fin 4) K)
    ...
    (hJ : MvPolynomial.weightedTotalDegree ![0,1,1,1] Q≤744) :
    Q.degreeOf 1≤744 ∧ Q.degreeOf 2≤244 ∧ Q.degreeOf 3≤122 := by
...
theorem hard_hereditary_common_cap {I : Type}
  ...
  · rw [hn]
    simpa only [n,w,a,v,small,
      ...
      WeightedIdentityExtendedLShapeArithmeticW1332266900.small] using
      WeightedIdentityExtendedLShapeArithmeticW1332266900.uniform_shortened_Johnson_gate.1
  · exact hdegree
  · exact hagreement
  · exact hS
  · exact hlarge
...
theorem activeTRegular_card_le_joint {I : Type}
    ...
    (hagreement : ∀ P∈Gamma,
      a≤(nodes.filter fun i => P.eval (x i)=u i).card) :
    (activeTRegularPolynomials Q Gamma).card≤247945303343219481 := by
  have hcommon := hard_hereditary_common_cap Gamma nodes x u hinj hn
    hdegree hagreement
  have hactive :=
    WeightedExtendedLShapeClosedW1332266900.extended_lshape_active_card_le
      Q hQ hT hRT hJ Gamma nodes x u hinj
      (by simpa only [n] using hn) hdegree hagreement hcommon
  have hcover : activeTRegularPolynomials Q Gamma⊆
      (positiveTFactors Q).biUnion
        (fun F =>
          WeightedIdentityExtendedLShapeIntegrationW1332266900.regularFamily
            F Gamma) := by
    intro P hP
  ...
  exact hcard.trans hactive

/-- Exact full-family bound in the hard W133226 branch. -/
theorem hard_scalar_card_le {I : Type} [Fintype I]
    ...
    (hagreement : ∀ P∈Gamma,
      a≤(nodes.filter fun i => P.eval (x i)=u i).card) :
    Gamma.card≤263552381097916929 := by
...
theorem hard_scalar_card_lt_coreFloor {I : Type} [Fintype I]
    ...
    (hagreement : ∀ P∈Gamma,
      a≤(nodes.filter fun i => P.eval (x i)=u i).card) :
    Gamma.card<
      WeightedIdentityExtendedLShapeArithmeticW1332266900.coreFloor := by
  exact (hard_scalar_card_le Q hQ hT hRT hJ Gamma nodes x u hinj hn
    hdegree hsolution hagreement).trans_lt (by
      norm_num [WeightedIdentityExtendedLShapeArithmeticW1332266900.coreFloor])
...
theorem hard_scalar_coreFloor_contradiction {I : Type} [Fintype I]
    ...
    (hagreement : ∀ P∈Gamma,
      a≤(nodes.filter fun i => P.eval (x i)=u i).card)
    (hlarge : WeightedIdentityExtendedLShapeArithmeticW1332266900.coreFloor≤
      Gamma.card) : False := by
...
end ProximityPrize.SubmissionLower.WeightedExtendedLShapeScalarClosureW1332266900
