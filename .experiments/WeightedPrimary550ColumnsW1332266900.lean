...
/-!
# Source-column receipt for the W=133226 identity-core helper

The exact quadratic pair numerator is evaluated symbolically.  Its quotient
by `2*133226` gives the conservative column lower bound used by the kernel
interface; no large finite computation is evaluated by the kernel.
-/
namespace ProximityPrize.SubmissionLower.WeightedPrimary550ColumnsW1332266900
...
def helperPairWidth (t r : Nat) : Nat :=
  99227150-(133225*r+133224*t)

def helperPairPolynomialSum : Nat :=
  genericPairPolynomialSum 99227150 133226 244

theorem helper_pair_cost_le (t r : Nat) (ht : t≤122) (hr : r≤244-2*t) :
    133225*r+133224*t≤99227150 := by
  omega

theorem helperPairWidth_cast (t r : Nat) (ht : t≤122) (hr : r≤244-2*t) :
    (helperPairWidth t r:ℚ)=99227150-133225*r-133224*t := by
...
theorem rational_helper_pair_inner (t : Nat) (ht : t≤122) :
    3*(∑ r ∈ Finset.range (244-2*t+1),
      (((99227150:ℚ)-133225*r-133224*t)^2+
        133226*((99227150:ℚ)-133225*r-133224*t)))=
      5133584822877750750-42972070571390215*t+
        13044445476354*(t:ℚ)^2-35497801256*(t:ℚ)^3 := by
  have hsafe : 2*t≤244 := by omega
  calc
    _=3*(∑ r ∈ Finset.range (244-2*t+1),
      (((99227150-133224*(t:ℚ))^2+133226*(99227150-133224*t))+
        (-266450*(99227150-133224*t)-17749033850)*(r:ℚ)+
...
theorem helperPairPolynomialSum_eq :
    helperPairPolynomialSum=105002019139681453177 := by
  have hcast : (helperPairPolynomialSum:ℚ)=
      ∑ t ∈ Finset.range 123,∑ r ∈ Finset.range (244-2*t+1),
        (((99227150:ℚ)-133225*r-133224*t)^2+
          133226*((99227150:ℚ)-133225*r-133224*t)) := by
    ...
    apply Finset.sum_congr rfl
    intro r hr
    rw [show WeightedSourceIndex6900.pairWidth 99227150 133226 r t =
  ...
  have hsum :
      3*(∑ t ∈ Finset.range 123,∑ r ∈ Finset.range (244-2*t+1),
        (((99227150:ℚ)-133225*r-133224*t)^2+
          133226*((99227150:ℚ)-133225*r-133224*t)))=
        (3*105002019139681453177:ℚ) := by
    calc
      _=∑ t ∈ Finset.range 123,
        (5133584822877750750+(-42972070571390215)*(t:ℚ)+
          13044445476354*(t:ℚ)^2+(-35497801256)*(t:ℚ)^3) := by
        rw [Finset.mul_sum]
        apply Finset.sum_congr rfl
        intro t ht
...
/-- Conservative integer column receipt obtained by dividing the exact pair
numerator by `2*W`. -/
theorem helper_columns_lower :
    394074801989406≤columns 99227150 133226 244 := by
  have hbase : helperPairPolynomialSum≤
      (2*133226)*columns 99227150 133226 244 := by
    exact pairPolynomialSum_le_columns 99227150 133226 244 (by norm_num)
  rw [helperPairPolynomialSum_eq] at hbase
  norm_num only [Nat.reduceMul] at hbase
  have hn : 266452*394074801989406≤105002019139681453177 := by norm_num
  have hc : 266452*394074801989406≤
      266452*columns 99227150 133226 244 := hn.trans hbase
  exact cancel_nat_mul_le 266452 394074801989406
    (columns 99227150 133226 244) (by norm_num) hc

#print axioms rational_helper_pair_inner
#print axioms helperPairPolynomialSum_eq
#print axioms helper_columns_lower

end ProximityPrize.SubmissionLower.WeightedPrimary550ColumnsW1332266900
