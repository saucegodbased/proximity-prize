import WeightedPrimary550KernelW1332266900
import Order2SourceSpecializationScaffold

/-! Universal vanishing produced by the exact hard W=133226 primary source. -/
namespace ProximityPrize.SubmissionLower.WeightedPrimary550UniversalW1332266900
...
theorem primary_universal_vanishing {K I : Type*} [Field K] [Fintype I]
    ...
    ∃ Q : Poly4 K, Q≠0 ∧ Q∈weightedCoefficientBox K 99227150 133226 244 ∧
      Q∈globalOrder2CoefficientBox K 99227150 133226 744 244 122 ∧
      ∀ (P : Polynomial K) (support : Finset I), P.natDegree≤133226 →
  ...
  obtain ⟨Q,hQ,hweighted,hcaps,hcontact⟩ :=
    WeightedPrimary550KernelW1332266900.primary_source_exists
      K I h2 nodes values hn
  refine ⟨Q,hQ,hweighted,hcaps,?_⟩
  intro P support hP hcard hvalues
  apply localJetSpecialization2_eq_zero_of_contact_and_degree
    Q P nodes values support 550 hinj.injOn h2
  · intro i hi
    exact hcontact i
  · exact hvalues
  · have hdegree := localJetSpecialization2_natDegree_lt
      99227150 133226 744 244 122 Q P (by norm_num) hcaps hP
    have hmany : 99227150≤550*support.card := by omega
    exact hdegree.trans_le hmany

#print axioms primary_universal_vanishing

end
end ProximityPrize.SubmissionLower.WeightedPrimary550UniversalW1332266900
