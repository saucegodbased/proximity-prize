import WeightedTrianglePrefixFormulaW1332246900

/-! Closed evaluation of the relaxed active-fibre count. -/
namespace ProximityPrize.SubmissionLower.WeightedRelaxedFibreFormulaW1332266900
...
theorem ceilWidth_relaxed_formula (C q s E n : Nat)
    (hspos : 0<s) (hsW : s<133226) (hE : 2*E<133226)
    (hnE : n≤E) (hC : C=q*133226+s) :
    ceilWidth 133226 (C-133224*n)=
      (if n≤q then q+1-n else 0)+
        (if n≤q+1 ∧ 133226<s+2*n then 1 else 0) := by
  unfold ceilWidth
  by_cases hnq : n≤q
  · have hcost : 133224*n≤C := by omega
    have hx : C-133224*n=(q-n)*133226+(s+2*n) := by
      have hsub := Nat.sub_add_cancel hcost
      have hqn := Nat.sub_add_cancel hnq
      omega
    rw [if_pos hnq,hx]
    by_cases hwrap : 133226<s+2*n
  ...
  · have hqn : q<n := Nat.lt_of_not_ge hnq
    ...
    · have heq : n=q+1 := by omega
      subst n
      by_cases hwrap : 133226<s+2*(q+1)
      · rw [if_pos ⟨le_rfl,hwrap⟩]
        have hcost : 133224*(q+1)≤C := by
          rw [hC]
          omega
        have hx : C-133224*(q+1)=s+2*(q+1)-133226 := by
          have hsub := Nat.sub_add_cancel hcost
          omega
        rw [hx]
        apply Nat.div_eq_of_lt_le
        ...
        · omega
      · rw [if_neg (by simpa only [not_and_or] using Or.inr hwrap)]
        have hzero : C≤133224*(q+1) := by rw [hC]; omega
        rw [Nat.sub_eq_zero_of_le hzero]
    · rw [if_neg (by omega)]
      have hzero : C≤133224*n := by
        rw [hC]
        omega
      rw [Nat.sub_eq_zero_of_le hzero]
...
theorem triangleRelaxedCount_cast_closed (C E : Nat)
    (hrem : 0<C%133226) (hE : 2*E<133226) :
    (triangleRelaxedCount C 133226 E:ℚ)=relaxedClosedQ C 133226 E := by
  let q:=C/133226
  let s:=C%133226
  let h:=(133226-s)/2+1
  have hsW : s<133226 := Nat.mod_lt C (by norm_num)
  have hC : C=q*133226+s := by
    dsimp [q,s]
    have hd := Nat.div_add_mod C 133226
    omega
  have hthreshold (n : Nat) : 133226<s+2*n ↔ h≤n := by
    dsimp [h,s]
    omega
  ...
  have hdecomp :
    (∑ n ∈ Finset.range (E+1),
      triangleMultiplicity E n*ceilWidth 133226 (C-133224*n)) =
      (∑ n ∈ Finset.range (E+1),
...
end ProximityPrize.SubmissionLower.WeightedRelaxedFibreFormulaW1332266900

#print axioms ProximityPrize.SubmissionLower.WeightedRelaxedFibreFormulaW1332266900.ceilWidth_relaxed_formula
#print axioms ProximityPrize.SubmissionLower.WeightedRelaxedFibreFormulaW1332266900.triangleRelaxedCount_cast_closed
