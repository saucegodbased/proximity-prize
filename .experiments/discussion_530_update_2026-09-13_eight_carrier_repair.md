...
Independent scaling evidence: in the one-error chambers `(n,w,g,e)=(4,1,3,1)`, with multiplicities `m=4,5,6,8`, the grade-`<=J` source plus only these eight families and three coefficient jets closes all three exact normals every time; `J2` is unnecessary.
